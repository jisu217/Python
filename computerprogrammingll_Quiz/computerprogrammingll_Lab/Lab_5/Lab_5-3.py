...
num = -1
while num != 0 :
    # num != 0: 사용자가 0을 입력할 때까지 루프를 계속 돌리기 위해 사용
    # num == 0이 되면, 더 이상 메뉴 선택을 진행하지 않으므로 루프가 끝나야 하기 때문
    ...
    # 종료 문구 출력
        ...
        # 사용자가 입력한 번호는 1부터 시작하지만, 파이썬의 리스트는 0부터 시작
        # num-1을 통해 사용자 입력을 리스트의 올바른 인덱스에 맞춰주는 것, 리스트에서 항목을 "거꾸로" 가져오는 게 아님
